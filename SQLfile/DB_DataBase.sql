...
INSERT INTO `user_dtl` (`id`, `phone`, `address`, `create_date`, `update_date`) VALUES
	...
	(2, NULL, NULL, '2022-12-09 11:21:52', '2022-12-09 11:21:52'),
...
DELIMITER //
CREATE TRIGGER `user_mst_before_delete` AFTER DELETE ON `user_mst` FOR EACH ROW BEGIN
	DELETE
	FROM
		user_dtl
	WHERE
...
-- 트리거 db_study.user_mst_before_insert 구조 내보내기
DROP TRIGGER IF EXISTS `user_mst_before_insert`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `user_mst_before_insert` AFTER INSERT ON `user_mst` FOR EACH ROW BEGIN
	INSERT INTO
		user_dtl
